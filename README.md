...
flowchart TD
    ...
    subgraph P2["🌡️ Phase 2: Structural Verification (Geometric Filter)"]
        direction TB
        sgRNA ---> Folding["3D Forward Folding (RhoFold/AlphaFold 3 Logic)"]
        Folding ---> Sim["Biophysical Simulation (Explicit Coordinates)"]
        Sim ---> Score["Alignment Scoring (RMSD / TM-Score Check)"]
        Score ---> PDB["PDB Conformation Filter"]
    end
...
### 🌡️ Phase 2: 3D Forward Folding & Structural Validation (The Geometric Filter)
* **Purpose**: Proves that the 1D text strings generated by the C-DDM can physically fold into a stable 3D conformation and dock perfectly into the Cas9 ribonucleoprotein complex (inspired by the **gRNAde** evaluation framework).
* **Inputs**: 1D de novo sgRNA text strings from Phase 1.
* **Processing Engines**:
  * *3D Structure Prediction*: Sequences are folded using 3D structural models (e.g., RhoFold or AlphaFold 3 logic) to predict explicit spatial coordinates ($x, y, z \in \mathbb{R}^3$) for the RNA backbone atoms.
  * *Alignment Scoring*: The predicted conformation is structurally aligned against the native, ideal Cas9-bound sgRNA conformation.
* **Output**: A structured `.pdb` coordinate file. Root Mean Square Deviation (**RMSD**) and **TM-score** are computed; sequences forming unstable folds or failing conformational constraints are computationally eliminated.
