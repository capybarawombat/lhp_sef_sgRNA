...
## 💎 Architectural Novelty & Advantages: Real-Time Gradient Steering vs. RL

To achieve clinical specificity, MAGE-CRISPR abandons traditional trial-and-error Reinforcement Learning (RL) paradigms in favor of real-time, biophysically guided generation:

* **Real-Time Logit Steering vs. Post-Generation Rejection**: Traditional RL-based CRISPR generators rely on post-generation rejection sampling—generating a complete sequence, evaluating it via reward functions or oracles, and discarding it if it fails. This process wastes massive amounts of GPU compute on "garbage outputs." In contrast, **Real-Time Gradient-Based Steering** guides the model *during* the generation process, eliminating invalid candidates before they are ever fully synthesized.
* **Unprecedented Sample Efficiency**: By using a differentiable thermodynamic surrogate function to calculate binding energy gradients ($\nabla_x E$) at each denoising timestep $t$, the generator directly adjusts its token probability logits. Every candidate is natively optimized for target specificity on the first forward pass.
* **Intimidating Computational Speed**: Bypassing the multi-step cycle of sequence generation, 3D folding, and downstream oracle evaluation loops for policy optimization accelerates candidate design from an expensive iterative search to a high-speed, single-pass inference task.
...
│   │   ├── feedback_agent.py    # Real-time thermodynamic gradient calculator & logit steering agent
│   │   └── train.py             # Denoising Markov step training loops & energy-guided steering
│   │
...
* **Real-Time Gradient-Based Steering (Energy-Guided Discrete Diffusion)**:
  - At each denoising timestep $t$, the Discrete Diffusion Model uses a differentiable thermodynamic surrogate function to calculate binding energy gradients ($\nabla_x E$) relative to the healthy wild-type allele.
  - This gradient acts as a mathematical force vector that directly adjusts the model's token probability logits at each step of the reverse Markov transition.
  - By actively steering the generation trajectory away from the healthy wild-type allele in real-time, the model natively engineers the required compensatory mismatches.
  - This forces the AI to natively engineer compensatory mismatches without needing a trial-and-error loop, post-generation negative sampling, or a constraint cache.
* **Thermodynamic Mismatch Engineering**:
  ...
  - By steering the trajectory away from wild-type binding using the energy gradient, the resulting guide is optimized to cleave the mutant (1 total mismatch) but thermodynamically detach from the healthy allele (2 total mismatches), maximizing allele selectivity.
...
* **Downstream Oracle Validation Filtering**:
  - Rather than serving as iterative reinforcement learning (RL) reward functions or updating a constraint cache, Evo 2 and STRAND act as the final validation oracles.
  - sgRNAs failing to meet Evo 2 zero-shot off-target thresholds or STRAND transcriptomic recovery marks are permanently filtered out.
  - This pipeline layout ensures that the generative engine produces high-quality candidates natively, while the downstream filters act as independent, zero-shot validation gates for clinical safety and efficacy.
